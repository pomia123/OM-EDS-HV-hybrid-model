...
def analyze_single_image(row_data, image_dir, vis_dir, pixel_size, save_vis=False):
    """
    Wrapper function to process a single image row from the dataset.
    ...
    try:
        ...
        if save_vis:
            ...
            print(f"\n[INFO] Visualization figures successfully saved to: {vis_dir}")
...
class MicrostructureFeatureExtractor:
    """
    A comprehensive feature extraction pipeline for Optical Microscopy (OM) images
    to quantify microstructural characteristics including secondary phases, eutectic structures,
    dendrite orientation, Dendrite Arm Spacing (DAS), and texture profiles.
    """
    ...
    def __init__(self, image_path, pixel_size=1.0):
        ...
        # Read image using numpy to bypass path encoding issues
        ...
        # Cached intermediate properties for multi-step visualization
    ...
    def _apply_clahe(self, bgr_img, clahe_clip=1.2):
        """Applies Contrast Limited Adaptive Histogram Equalization (CLAHE) in HSV space."""
    ...
    def build_orientation_map(self, input_gray=None):
        """Generates a binary mask using Sauvola local thresholding for orientation analysis."""
    ...
    def extract_all_features(self):
        """Executes all microstructural extraction modules and consolidates the feature dictionary."""
    ...
    # -------------------------------------------------------------------------
    # 1. Secondary Phase & Porosity Quantification
    # -------------------------------------------------------------------------
    def extract_phase_features(self):
        """Quantifies geometric features of secondary phases or pores via Otsu thresholding."""
        features = {}
        ...
        self._binary_phase = binary_phase  # Cache for visualization
        ...
        if len(props_phase) > 0:
            ...
            features['phase_count_density'] = len(props_phase) / total_pixels
            ...
            features['phase_area_cv'] = (np.std(areas_phase) / np.mean(areas_phase)
                                         if np.mean(areas_phase) > 0 else 0.0)
        else:
    ...
    # -------------------------------------------------------------------------
    # 2. Eutectic Structure Analysis (Lamella Thickness)
    # -------------------------------------------------------------------------
    def estimate_eutectic_spacing(self, target_dark_rgb=[27, 34, 52], otsu_weight=1.1, dark_margin=40):
        """Estimates eutectic lamella thickness using distance transform and skeletonization."""
        features = {}
        ...
        # Masking dark/background artifacts
        ...
        # Distance transform to compute lamella width profiles
        ...
        if samples.size > 0:
            ...
            # Note: Maps to physical thickness unit using pixel scale factors
    ...
    # -------------------------------------------------------------------------
    # 3. Microstructural Orientation & Dendrite Arm Spacing (DAS)
    # -------------------------------------------------------------------------
    def extract_orientation_features(self):
        """Calculates directional field orientation parameters and estimates DAS via linear interception."""
        features = {}
        ...
        if np.sum(valid_mask) > 0:
            ...
            # Cache properties for visualization module
        ...
        # --- Dendrite Arm Spacing (DAS) Analysis via Virtual Intercept Lines ---
        ...
        def _sample_one_angle(binary_img, angle_deg, n_lines=40):
            ...
            for offset in offsets:
                ...
                # Boundary exclusion: Skip cropped structures at image edges
                if profile[0] == 1 or profile[-1] == 1:
                    continue

                n_trans = int(transitions.sum())
                if n_trans >= 2:
                    # Calculate intercept frequency strictly inside validated boundaries
    ...
    # -------------------------------------------------------------------------
    # 4. Global Gray-Level Texture Metrics
    # -------------------------------------------------------------------------
    def extract_texture_features(self):
        """Extracts statistical parameters, GLCM texture markers, and LBP distributions."""
        features = {}
    ...
    def calculate_glcm_features(self):
        """Computes Gray-Level Co-occurrence Matrix (GLCM) statistical features."""
        features = {}
        gray_reduced = (self.gray // 4).astype(np.uint8)  # Quantize intensity to 64 levels
        distances = [1, 3, 5]
    ...
    def calculate_lbp_features(self):
        """Quantifies texture uniformities via Local Binary Patterns (LBP)."""
        features = {}
        radius = 3
        n_points = 8 * radius
        ...
        # Ratio of uniform patterns within the calculated domain descriptor
        features['lbp_uniformity'] = float(np.sum(lbp < n_points + 1) / lbp.size)

        return features

    # -------------------------------------------------------------------------
    # 5. Spatial Autocorrelation Length Analysis
    # -------------------------------------------------------------------------
    def _calc_autocorr_profile(self, img=None, axis='h'):
        """Calculates spatial autocorrelation tracking length decay via 2D Fast Fourier Transform (FFT)."""
        if img is None:
            img = self.gray
        std_val = np.std(img)
        if std_val == 0:
        ...
        try:
            # Locate threshold boundary index down to 1/e (~36.8% decay)
    ...
    def calculate_autocorrelation_length(self, img=None):
        """Legacy wrapper matching previous structural variants."""
    ...
    # =========================================================================
    # Visualization Pipeline (Publication-ready: 300 DPI, clean white layout)
    # =========================================================================

    def _save_img(self, img_data, output_path, cmap=None):
        """Saves matrix structures directly as image files without titles or axis ticks."""
    ...
    def _save_plot(self, plot_fn, output_path):
        """Saves graphical profiles or statistical histograms as standalone plots."""
    ...
    def visualize_phase_features(self, output_path='phase_features.png'):
        """Generates and exports target segmented masks and phase size distribution metrics."""
    ...
    def visualize_eutectic_features(self, output_path='eutectic_features.png'):
        """Saves spatial eutectic region boundaries and lamella thickness spatial distributions."""
    ...
    def visualize_orientation_das_features(self, output_path='orientation_das_features.png'):
        """Generates polar distribution fields and categorical DAS spatial metrics across fixed axes."""
        ...
        if hasattr(self, '_orientation_angles'):
            ...
            ax.set_thetamin(0)
            ax.set_thetamax(180)
            ax.bar(bin_centers, hist_norm, width=np.pi / 18, color='steelblue', alpha=0.8, edgecolor='none')
            ax.plot([mean_rad, mean_rad], [0, hist_norm.max()], color='tomato', lw=1.5)
            fig.savefig(f"{base}_03_polar_hist.png", dpi=300, bbox_inches='tight', facecolor='white', edgecolor='none')
            plt.close(fig)
            print(f"[Saved Plot] {base}_03_polar_hist.png")
        ...
        def plot_das(ax):
            ...
            ax.axhline(mean_das, color='tomato', lw=1.5, ls='--', label=f'Mean = {mean_das:.1f} px')
    ...
    def visualize_texture_features(self, output_path='texture_features.png'):
        """Plots Local Binary Patterns, signal intensity maps, and multi-distance GLCM markers."""
        ...
        def plot_intensity(ax):
            ...
            ax.axvline(mean_intensity, color='tomato', lw=1.5, label=f'Mean = {mean_intensity:.1f}')
        ...
        if self._glcm is not None:
            ...
            def plot_glcm_contrast(ax):
                contrast_mat = graycoprops(self._glcm, 'contrast')
                ...
                for i in range(3):
                    for j in range(4):
                        ax.text(j, i, f'{contrast_mat[i, j]:.2f}', ha='center', va='center', fontsize=font_size, color='black')
    ...
    def visualize_segmentation(self, output_path='comprehensive_analysis.png'):
        """Backward-compatible wrapper function for sequence evaluations."""
...
def process_microstructure_parallel():
    """Executes parallel microstructural characterization across the dataset repository."""
    base_dir = r'./'
    ...
    print(f"[INFO] Initiating multi-process feature extraction for {len(df)} samples...")
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for i, row in enumerate(rows_to_process):
            ...
            if not first_valid_submitted:
                should_save = True  # Save visualization artifacts only for the primary sample index
                first_valid_submitted = True
            futures[executor.submit(
                analyze_single_image, row, image_dir, vis_dir, 1.0, should_save
            )] = i

        for future in tqdm(as_completed(futures), total=len(futures), desc="Characterization Pipeline"):
            idx = futures[future]
            try:
    ...
    print(f"\n[INFO] Feature extraction pipeline completed successfully. Output saved to: {output_path}")
