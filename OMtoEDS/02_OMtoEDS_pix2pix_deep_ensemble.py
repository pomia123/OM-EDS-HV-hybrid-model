...
LAMBDAS_PIX      = [10.0, 10.0, 10.0, 10.0, 10.0, 15.0]
FOCAL_ALPHA_POS  = [0.7,  0.8,  0.7,  0.8,  0.7,  0.8]
FOCAL_GAMMA      = [3.0,  3.0,  3.0,  1.0,  1.0,  1.0]
LOSS_RATIO       = [0.7,  0.5,  0.3,  0.3,  0.5,  0.3]  # focal_w per element; tversky_w = 1 - focal_w
LAMBDA_GAN = 1.0
N_ENSEMBLE = 3  # Deep Ensemble: number of independently trained Generators per element
...
class MetallurgyDataset(Dataset):
    """
    Custom Dataset class with pre-cached percentile thresholding.
    Loads and augments paired OM and EDS images.
    """
    def __init__(self, file_list, target_idx=0, is_train=True):
        self.file_names = file_list
        self.target_idx = target_idx
        self.elem_name  = ELEM_NAMES[target_idx] if isinstance(target_idx, int) else target_idx
        self.suf        = EDS_SUFFIXES[target_idx] if isinstance(target_idx, int) else EDS_SUFFIXES[self.elem_name]
    ...
    def __getitem__(self, idx):
        ...
        raw = imread_korean(os.path.join(EDS_DIR, f"{base_name}_{self.suf}.png"), 0)
        
        th = self.th_cache[base_name]
...
def combined_loss(p_logit, t, valid_mask, elem_key, focal_alpha_pos, focal_gamma, loss_ratio):
    alpha, beta = TVERSKY_PARAMS[elem_key]
    focal_w, tversky_w = loss_ratio, 1 - loss_ratio
    return (focal_w   * focal_loss(p_logit, t, valid_mask, gamma=focal_gamma, alpha_pos=focal_alpha_pos) +
...
def calculate_sample_metrics(logit, target, mask, threshold=0.5):
    pred_bin = (torch.sigmoid(logit) > threshold).float() * mask
    target_bin = (target > 0.5).float() * mask
    ...
    valid_pixels = mask.sum().item() or 1e-8
    pred_area_pct = (pred_bin.sum().item() / valid_pixels) * 100.0
    gt_area_pct = (target_bin.sum().item() / valid_pixels) * 100.0
    
    mae = abs(pred_area_pct - gt_area_pct)
    rmse = ((pred_area_pct - gt_area_pct) ** 2) ** 0.5   
    mape = abs(pred_area_pct - gt_area_pct) / gt_area_pct * 100.0
...
if __name__ == '__main__':
    ...
    # num_workers = 12 if os.name != 'nt' else 0
    num_workers = 12

    def train_ensemble_member(elem_idx, key, elem_name, ensemble_idx, train_loader, val_loader,
                               is_al, lam_pix, focal_alpha_pos, focal_gamma, loss_ratio):
        """Trains one Generator/Discriminator pair (one Deep Ensemble member) for a given element."""
        LR_INIT = 0.0002
        ...
        print(f"\n[INFO] Ensemble member {ensemble_idx+1}/{N_ENSEMBLE} | Device: {device} | λ_pix={lam_pix}")
        ...
        for epoch in range(EPOCHS):
            ...
            for om, eds, v_mask, align_mask, _ in train_loader:
                ...
                with autocast():
                    ...
                    loss_pix = combined_loss(
                        fake_logit, eds, c_mask, elem_key=key,
                        focal_alpha_pos=focal_alpha_pos, focal_gamma=focal_gamma, loss_ratio=loss_ratio,
                    )
            ...
            if epoch % 5 == 0:
                ...
                if avg_val_iou > best_iou:
                    ...
                    torch.save(sd, os.path.join(MODEL_DIR, f"best_model_{key}_{ensemble_idx}_{epoch}.pth"))
            ...
            if epoch % 50 == 0:
                ...
                with torch.no_grad():
                    ...
                    sample_logit = G(sample_om)
                    pred_sig  = torch.sigmoid(sample_logit).cpu()

                mask_vis = sample_align[0, 0] if is_al else sample_mask[0, 0]
                mask_vis_np = mask_vis.numpy()
                metric_mask = (
                    sample_align[0:1].to(device) if is_al else sample_mask[0:1].to(device)
                )
                sample_metrics = calculate_sample_metrics(
                    sample_logit[0:1], sample_eds[0:1].to(device), metric_mask
                )
                ...
                metrics_row = {
                    ...
                    "epoch": int(epoch),
                    "train_iou": float(run_iou / max(1, len(train_loader))),
                    "val_iou": float(sample_metrics["iou"]),
                    "rmse": float(sample_metrics["rmse"]),
                    "mae": float(sample_metrics["mae"]),
                    "mape": float(sample_metrics["mape"]),
                    "mape_definition": "relative_area_pct_error",
                    "pred_area_pct": float(sample_metrics["pred_area_pct"]),
                    "gt_area_pct": float(sample_metrics["gt_area_pct"]),
                    "valid_pixels": sample_metrics["valid_pixels"],
                    "metric_scope": "first_validation_image",
                ...
                ax.imshow(om_img)
                ax.axis('off')
                plt.savefig(os.path.join(SAVE_DIR, f'pred_{key}_{ensemble_idx}_epoch_{epoch:04d}_OM.png'), dpi=300, bbox_inches='tight', pad_inches=0)
                plt.close()
                ...
                ax.axis('off')
                plt.savefig(os.path.join(SAVE_DIR, f'pred_{key}_{ensemble_idx}_epoch_{epoch:04d}_GT.png'), dpi=300, bbox_inches='tight', pad_inches=0)
                plt.close()
                ...
                ax.axis('off')
                plt.savefig(os.path.join(SAVE_DIR, f'pred_{key}_{ensemble_idx}_epoch_{epoch:04d}_Pred.png'), dpi=300, bbox_inches='tight', pad_inches=0)
                plt.close()

        metrics_path = os.path.join(SAVE_DIR, f"metrics_{key}_{ensemble_idx}.csv")

        if metrics_log:
            df = pd.DataFrame(metrics_log)
            df.to_csv(metrics_path, index=False, encoding="utf-8")

        print(f"[SAVED] Metrics (logged every 50 epochs) -> {metrics_path}")

        # Phase Completion & Final Weights Persistence
        final_path = os.path.join(MODEL_DIR, f"last_model_{key}_{ensemble_idx}.pth")
        sd = G.module.state_dict() if isinstance(G, nn.DataParallel) else G.state_dict()
        torch.save(sd, final_path)

        print(f"\n[SUCCESS] Ensemble member {ensemble_idx+1}/{N_ENSEMBLE} for {elem_name} completed successfully.")
        print(f"[INFO] Benchmark IoU: {best_iou:.4f} | Artifact registered at: {final_path}")

        G_eval = G.module if isinstance(G, nn.DataParallel) else G
        G_eval.eval().cpu()  
        del D, opt_G, opt_D, scaler_G, scaler_D
        torch.cuda.empty_cache()
        return G_eval, best_iou
    ...
    for elem_idx, (key, elem_name) in enumerate(zip(elem_keys, ELEM_NAMES)):
        ...
        val_loader = DataLoader(
            ...
            persistent_workers=(max(num_workers // 2, 0) > 0),
        )
        ...
        # Deep Ensemble: sequentially train N_ENSEMBLE independent Generators for this element
        is_al = (elem_name == "Al")
        lam_pix         = LAMBDAS_PIX[elem_idx]
        focal_alpha_pos = FOCAL_ALPHA_POS[elem_idx]
        focal_gamma     = FOCAL_GAMMA[elem_idx]
        loss_ratio      = LOSS_RATIO[elem_idx]

        ensemble_generators = []
        ensemble_best_ious  = []
        for ensemble_idx in range(N_ENSEMBLE):
            G_member, member_best_iou = train_ensemble_member(
                elem_idx, key, elem_name, ensemble_idx, train_loader, val_loader,
                is_al, lam_pix, focal_alpha_pos, focal_gamma, loss_ratio,
            )
            ensemble_generators.append(G_member)
            ensemble_best_ious.append(member_best_iou)

        best_ious[key] = max(ensemble_best_ious)

        # -----------------------------------------------------------------
        # Deep Ensemble Uncertainty Estimation (replaces MC Dropout)
        # -----------------------------------------------------------------
        print(f"\n[INFO] Computing Deep Ensemble uncertainty for {elem_name} ({N_ENSEMBLE} members)")
        with torch.no_grad():
            sample_om, sample_eds, sample_mask, sample_align, sample_name = next(iter(val_loader))
            sample_om = sample_om.to(device)
            
            preds = []
            for g in ensemble_generators:
                g.to(device)
                preds.append(torch.sigmoid(g(sample_om)))
                g.cpu()
                
            member_preds = torch.stack(preds, dim=0)
            ensemble_mean = member_preds.mean(dim=0).cpu()
            ensemble_std = member_preds.std(dim=0).cpu()

        mask_vis = sample_align[0, 0] if is_al else sample_mask[0, 0]
        mask_vis_np = mask_vis.numpy()

        # Ensemble mean prediction
        mean_img = ensemble_mean[0, 0].numpy() * mask_vis_np
        fig, ax = plt.subplots(figsize=(20, 20))
        ax.imshow(mean_img, cmap='gray', vmin=0, vmax=max(1.0, mean_img.max()))
        ax.axis('off')
        plt.savefig(os.path.join(SAVE_DIR, f'ensemble_{key}_mean.png'), dpi=300, bbox_inches='tight', pad_inches=0)
        plt.close()

        # Ensemble uncertainty (std across members)
        std_img = ensemble_std[0, 0].numpy() * mask_vis_np
        fig, ax = plt.subplots(figsize=(20, 20))
        im = ax.imshow(std_img, cmap='inferno')
        ax.axis('off')
        plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
        plt.savefig(os.path.join(SAVE_DIR, f'ensemble_{key}_uncertainty.png'), dpi=300, bbox_inches='tight', pad_inches=0)
        plt.close()

        ensemble_metrics_path = os.path.join(SAVE_DIR, f"ensemble_metrics_{key}.csv")
        pd.DataFrame([{
            "element": key,
            "n_ensemble": N_ENSEMBLE,
            "member_best_ious": ensemble_best_ious,
            "best_member_iou": max(ensemble_best_ious),
            "ensemble_uncertainty_mean": float(ensemble_std[0, 0].mean().item()),
            "sample_name": str(sample_name[0]),
        }]).to_csv(ensemble_metrics_path, index=False, encoding="utf-8")

        print(f"[SAVED] Deep Ensemble uncertainty artifacts -> {SAVE_DIR} (ensemble_{key}_mean.png, ensemble_{key}_uncertainty.png)")
        print(f"[SAVED] Ensemble metrics -> {ensemble_metrics_path}")
        print(f"\n[SUCCESS] Model optimization for {elem_name} completed successfully.")
        print(f"[INFO] Benchmark IoU (best of {N_ENSEMBLE} members): {best_ious[key]:.4f}")
