...
- Deep Ensemble: each element/tag has N_ENSEMBLE independently trained Generators
  (best_model_{key}_{idx}.pth / last_model_{key}_{idx}.pth). The mean of their sigmoid
  outputs is thresholded to produce the final binary prediction used in the overlay.
  No uncertainty output here — mean prediction only.
- Evaluates the 'best' and 'last' checkpoint ensembles to visualize elemental distributions simultaneously.
- Produces two versions of visual outputs:
  1. All 6 Elements (Al, Si, Mg, Fe, Cu, Sr)
  2. Precipitate Elements Only (Mg, Fe, Cu, Sr)
...
PREC_ELEMS  = ["Mg", "Fe", "Cu", "Sr"]   # Version excluding dominant phases (Al, Si)
THRESHOLD   = 0.5
N_ENSEMBLE  = 3
...
def evaluate_and_save_multi_overlay(models, loader, device, elem_names, save_dir, model_tag, legend_ncol):
    """
    Runs Deep Ensemble inference per batch (mean of member probabilities per element)
    and exports GT & Predicted multi-element overlays.
    """

    os.makedirs(save_dir, exist_ok=True)

    for member_list in models.values():
    ...
    with torch.no_grad():
        for om_t, eds_bins, v_mask, base_names, om_raw_crops in loader:
            ...
            # Batch-wise ensemble inference for each element: mean of member probabilities
            pred_bins_batch = {}
            for en in elem_names:
                member_probs = [torch.sigmoid(G(om_t)) for G in models[en]]
                mean_prob = torch.stack(member_probs, dim=0).mean(dim=0).cpu().numpy()
                pred_bins_batch[en] = (mean_prob[:, 0] > THRESHOLD).astype(np.uint8)
...
def main():
    ...
    for model_tag in ["best", "last"]:
        ...
        avail_all = {en: load_model_ensemble(en, model_tag, device) for en in ALL_ELEMS}
        avail_all = {en: m for en, m in avail_all.items() if m}
        ...
        for member_list in avail_all.values():
            for G in member_list:
                G.cpu()
        del avail_all
        ...
        avail_prec = {en: load_model_ensemble(en, model_tag, device) for en in PREC_ELEMS}
        avail_prec = {en: m for en, m in avail_prec.items() if m}
        ...
        for member_list in avail_prec.values():
            for G in member_list:
                G.cpu()
        del avail_prec
        torch.cuda.empty_cache()
